        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for i in range(len(fx)):
                    ...
                    logits = torch.add(logits,client_logits)
    ...
    def get_kd_logits(self, net, net_a_list):
        ...
        with torch.no_grad():
            for i in range(len(self.data)):
                ...
                auxnet = net_a_list[0]
                auxnet.eval()
                logits,_ = auxnet(self.data[i])
                for i in range(1,len(fx_server)):
                    auxiliary_net = net_a_list[i]# fx[i][1]-1]
                    auxiliary_net.eval()
                    server_logits, _ = auxiliary_net(fx_server[i-1])
                    logits = torch.add(logits ,server_logits)
                logits = torch.add(logits, fx_server[-1].detach())
