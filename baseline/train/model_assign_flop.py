from model.resnet_hetero_flop import *
from model.resnet_hetero_flop_v2 import resnet18_FL_v2, resnet18_depthFL_v2
from model.resnet_hetero_flop_v3 import resnet18_FL_v3, resnet18_depthFL_v3
from model.resnet_hetero_flop import *
...
# FL인데 크기가 줄어든 FL입니다
def FL_model_assignment(model_v, cut_point, model_name, device, num_classes = 10):
    if model_v == 1:
        cut_points = [1,2,3]
        ...
        cut_points = [1,2,3]
        if model_name == 'resnet18':    # 확인
            net_glob = resnet18_FL_v2(num_classes, cut_point).to(device)
        elif model_name == 'resnet34':  # 확인
            net_glob = resnet34_FL(num_classes, cut_point).to(device)
        elif model_name == 'resnet50':
            net_glob = resnet50_FL(num_classes, cut_point).to(device)
        ...
        cut_points = [1,2,3,4]
        if model_name == 'resnet18':    # 확인
            net_glob = resnet18_FL_v3(num_classes, cut_point).to(device)
        elif model_name == 'resnet34':  # 확인
            net_glob = resnet34_FL(num_classes, cut_point).to(device)
        elif model_name == 'resnet50':
            net_glob = resnet50_FL(num_classes, cut_point).to(device)
...
def depth_model_assignment(model_v, model_name, num_classes, device):
    local_models = []
    auxiliary_models = []
    if model_v == 1:
        cut_point = [1,2,3,5]
        ...
        if model_name == 'resnet18':
            for i in range(len(cut_point)):
                    local_models.append(resnet18_depthFL_v2(num_classes, cut_point[i]))
        elif model_name == 'resnet34':
            for i in range(len(cut_point)):
                    local_models.append(resnet34_depthFL(num_classes, cut_point[i]))
        elif model_name == 'resnet50':
        ...
        if model_name == 'resnet18':
            for i in range(len(cut_point)):
                    local_models.append(resnet18_depthFL_v3(num_classes, cut_point[i]))
        elif model_name == 'resnet34':
            for i in range(len(cut_point)):
                    local_models.append(resnet34_depthFL(num_classes, cut_point[i]))
        elif model_name == 'resnet50':
