...
class ResNet101_server_v1(nn.Module):  # Dropout (or pruned) ResNet101 [width] 
    ...
    def forward(self, x):
        out1 = self.layer3[0:8](x)
        out2 = self.layer3[8:19](out1)
        out = self.layer3[19:24](out2)
        out = self.layer4(out)
...
class ResNet101_client_v2(nn.Module):  # Dropout (or pruned) ResNet [width] 
    ...
    def forward(self, x):
        ...
        out2 = self.layer1(out1)
        out3 = self.layer2(out2)
        out4 = self.layer3[0:1](out3)   # 확인필요
        out5 = self.layer3[1:](out4)
        return [out4, out5]
...
class ResNet101_server_v2(nn.Module):  # Dropout (or pruned) ResNet101 [width] 
    ...
    def forward(self, x):
        out1 = self.layer3[0:11](x)
        out = self.layer3[11:](out1)
        out = self.layer4(out)
        out = F.avg_pool2d(out, out.size()[3])
        out = out.view(out.size(0), -1)
        logits = self.linear(out)
...
class ResNet101_client_v3(nn.Module):  # Dropout (or pruned) ResNet [width] 
    def __init__(self, block, num_blocks,  num_classes=10):
    ...
    def forward(self, x):
        ...
        out5 = self.layer3[1:9](out4)
        out6 = self.layer3[9:](out5)
