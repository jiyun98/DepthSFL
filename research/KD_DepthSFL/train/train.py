# multiple smashed data + depthfl + server-side distillation
import torch
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for i in range(len(fx)):
                    auxiliary_net = aux_client[i]
                    auxiliary_net.zero_grad()
                    client_logits, client_probs = auxiliary_net(fx[i])
                    CE_loss_client += criterion_CE(client_logits, labels)
                    acc_client = calculate_accuracy(client_logits, labels)  
                # KD loss
                fx_client = fx[-1].clone().detach().requires_grad_(True)

                net_server , aux_server, server_logits, self.args, loss_server, acc_server= train_server(fx_client, aux_server, labels, net_server, self.args)

                if self.args.kd_opt:
                    KD_loss_client = criterion_KD(client_logits, server_logits.detach()) 
                    loss_client = CE_loss_client + self.args.lambdaa * KD_loss_client
                else:
                     loss_client = CE_loss_client
...
def train_server(fx_client, aux_server, y, net, args, loss_opt = False): 
    ...
    fx_server2 = fx_server[-1].clone()
    acc = calculate_accuracy(probs, y)  
    loss.backward()
    optimizer_server.step()

    return net_server, copy.deepcopy(net_ax), fx_server2, args, loss, acc
...
def test_img(net_c, net_s, net_a, datatest, args):
    ...
    with torch.no_grad():
      for idx, (data, target) in enumerate(data_loader):
            ...
            # server-side loss
            logits_s, probas_s = net_s(fx_client[-1])# ,fx_client[-1][1])
