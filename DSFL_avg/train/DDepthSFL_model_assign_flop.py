from model.model_DDepthSFL.resnet_hetero_flop import *
from model.model_DDepthSFL.resnet_hetero_flop_v2 import *
from model.model_DDepthSFL.resnet_hetero_flop_v3 import *
from model.model_DDepthSFL.auxnet import Aux_net_v2, Aux_net
import copy


def model_assignment(model_v, model_name, num_classes, device):
    local_cmodels = []
    local_smodels = []
    auxiliary_models = []
    if model_v == 1:
        ...
        if model_name == 'resnet18':
            for i in range(len(cut_point)):
                    local_cmodels.append(resnet18_client_v2(num_classes, cut_point[i]))
                    local_smodels.append(resnet18_server_v2(num_classes, cut_point[i]))
        elif model_name == 'resnet34':
            for i in range(len(cut_point)):
                    local_cmodels.append(resnet34_client(num_classes, cut_point[i]))
                    local_smodels.append(resnet34_server(num_classes, cut_point[i]))
        ...
        if model_name == 'resnet18':
            for i in range(len(cut_point)):
                    local_cmodels.append(resnet18_client_v3(num_classes, cut_point[i]))
                    local_smodels.append(resnet18_server_v3(num_classes, cut_point[i]))
        elif model_name == 'resnet34':
            for i in range(len(cut_point)):
                    local_cmodels.append(resnet34_client(num_classes, cut_point[i]))
                    local_smodels.append(resnet34_server(num_classes, cut_point[i]))
...
def global_model_assignment(model_v, cut_point, model_name, device, num_classes = 10):
    if model_name == 'resnet18':
