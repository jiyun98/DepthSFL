...
def main_DDepthSFL(args):      
    ...
    c_auxiliary_models = copy.deepcopy(auxiliary_models)
    s_auxiliary_models = copy.deepcopy(auxiliary_models)
    args.num_models = len(args.cut_point)
    
    # Global model definition
    #net_glob_client, net_glob_server = global_model_assignment(args.model_v,args.model_name, args.device, args.num_classes)
    ...
    for iter in range(1,args.epochs+1):
        ...
        if iter > args.kd_epoch and args.kd_server_opt:
            kd_server_opt =True    
        ...
        for idx in idxs_users:
            ...
            aux_client = copy.deepcopy(c_auxiliary_models[:model_idx+1])
            aux_server = copy.deepcopy(s_auxiliary_models[model_idx:])
            ...
            if kd_server_opt:
                kd_logits = local_s.get_kd_logits(net = copy.deepcopy(s_model_select))
                weight_c, weight_a_c,  args, loss_c, acc_c = local_c.train(
                    net_client = copy.deepcopy(c_model_select), net_ax = copy.deepcopy(aux_client), kd_server_opt = kd_server_opt , kd_logits = kd_logits)
            else:
                weight_c, weight_a_c,  args, loss_c, acc_c = local_c.train(
            ...
            for i in range(args.num_models):
                if i < model_idx: # [0,1,2,3] 가정 model_idx = 1 -> [0,1] [1,2,3]
                    w_locals_a_c[i].append(weight_a_c[i])
                elif i == model_idx:
                    w_locals_a_c[i].append(weight_a_c[i])
                    w_locals_a_s[i].append(weight_a_s[i-model_idx])
                elif i > model_idx : 
                    w_locals_a_s[i].append(weight_a_s[i-model_idx])
        ...
        c_auxiliary_models = HeteroAvg_auxnet(w_locals_a_c, c_auxiliary_models)
        s_auxiliary_models = HeteroAvg_auxnet(w_locals_a_s, s_auxiliary_models)
        ...
        if iter % 10 == 0:
            ...
            for ind in range(ti):
                ...
                model_e_a = copy.deepcopy(c_auxiliary_models[ind])
    ...
    file_name_c = './output_avg2/{}/'.format(args.method_name) + args.name + '/[client]test_accuracy.txt'
    file_name_s = './output_avg2/{}/'.format(args.method_name) + args.name + '/[server]test_accuracy.txt'
