# flops수 기준으로 모델 분할 : 1%, 15%, 28%, 39%,52%
...
class ResNet18_server_v2(nn.Module):  # Dropout (or pruned) ResNet [width] 
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], 1, layer1,  1, True)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.linear = nn.Linear(512*block.expansion, num_classes)
...
# ---------------------------------------------------
class ResNet18_client_v4(nn.Module):  # Dropout (or pruned) ResNet [width] 
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet18_client_v4, self).__init__()
        self.in_planes = 64
...
class ResNet18_server_v4(nn.Module):  # Dropout (or pruned) ResNet [width] 
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet18_server_v4, self).__init__()
        self.in_planes = 64
